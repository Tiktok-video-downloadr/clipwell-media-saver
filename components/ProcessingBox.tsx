...
export function ProcessingBox() {
  ...
  async function handleFileSelected(file: File) {
    ...
    const jobRes = await fetch("/api/jobs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ kind: "upload", uploadRef, targetFormat: "mp4" }),
    });

    if (!jobRes.ok) {
      const body = await jobRes.json().catch(() => ({}));
      setJob({ phase: "error", errorMessage: body.message ?? "Could not start processing." });
      return;
    }
      ...
      <div role="status" className="mt-4 rounded-md border border-[var(--line)] bg-white p-4 text-sm">
        {job.thumbnailUrl && (
          <img
            ...
            alt=""
            className="mb-3 w-full max-w-[240px] rounded-md border border-[var(--line)]"
          />
        )}
